...
## Утилита ripgrep (rg) вместо классического grep используется по нескольким практическим причинам:
- Скорость работы: ripgrep написан на Rust и является одной из самых быстрых утилит поиска в мире.
- Умная фильтрация: В отличие от grep, он автоматически исключает сам себя из результатов поиска (вам не нужно дописывать | grep -v grep).
- Цветной вывод: По умолчанию наглядно подсвечивает найденные совпадения в терминале.
- Современный синтаксис: Поддерживает регулярные выражения Perl (PCRE2) из коробки без дополнительных флагов.

## Спислк процессов: `ps aux` против `ps -ef`. Что выбрать?

### 1. Синтаксис флагов
* `ps -ef` использует дефис. Это стандартные флаги UNIX (POSIX).
* `ps aux` пишется без дефиса. В стиле BSD дефис перед флагами не ставился. Если вы случайно напишете `ps -aux`, Linux выдаст предупреждение или покажет другой результат.

### 2. Разница в выводимых колонках
Каждая команда показывает свой набор данных о процессах.

* `ps -ef` делает упор на связи процессов:
	* Показывает PPID (родительский процесс), что позволяет понять, какая программа запустила данный процесс.
	* Показывает UID (имя пользователя словом).
* `ps aux` делает упор на ресурсы системы:
	* Показывает %CPU (нагрузку на процессор прямо сейчас).
	* Показывает %MEM (процент использования оперативной памяти).
	* Показывает VSZ и RSS (точный объем виртуальной и физической памяти в килобайтах).
	* Показывает USER (имя пользователя) и STAT (текущее состояние процесса: спит, запущен, зомби).

### 3. Разница в охвате процессов
* -e в `ps -ef` - гарантированно выбирает вообще все процессы в системе. Без этого флага ps покажет только процессы текущего пользователя, привязанные к текущему терминалу
* -f в `ps -ef` - включает полный формат отображения данных. Он добавляет в таблицу важные колонки, такие как имя пользователя (UID), идентификатор родительского процесса (PPID), время старта и полный текст запущенной команды с аргументами.
* a+x в `ps aux` собирает процессы всех пользователей (a) и процессы, не привязанные к терминалу (x, то есть системные демоны). Итоговый список процессов у обеих команд практически идентичен, но фильтрация внутри ядра происходит по разным алгоритмам.
